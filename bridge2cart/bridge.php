...
+-----------------------------------------------------------------------------*/
/**
 * dump_action.php morozov 20.02.13 17:26
 */
...
class M1_Bridge_Action_Dump
{
  ...
  function perform(M1_Bridge $bridge)
  {
   ...
   */
  function _checkFolder(M1_Bridge $bridge, $viewsError = true)
  {
    $message = '';
    $type = 0;
...
#
# $Id: magentoerrorlog_action.class.php 479 2009-11-02 16:44:05Z peter $
#
...
#
if (!class_exists("M1_Bridge_Action_Savefile")) {
...
#
# $Id: carttype_action.class.php 479 2009-11-02 16:44:05Z peter $
#
...
class M1_Config_Adapter
{
  ...
  function create()
  {
    ...
    if (!class_exists("M1_Config_Adapter_" . $cartType)) {
      @include realpath(M1_Config_Adapter::getAdapterPath($cartType));
    }
  ...
  function detectCartType()
  {
     ...
     * Virtuemart113
     */
    if ((file_exists(M1_STORE_BASE_DIR . "configuration.php")) && (!file_exists(M1_STORE_BASE_DIR . '/components/com_mijoshop/mijoshop.php'))) {
    ...
    //OXID e-shop
    if( file_exists( M1_STORE_BASE_DIR . 'oxid.php') || file_exists( M1_STORE_BASE_DIR . '/core/oxid.php' ) ) {
      return 'Oxid';
    }
  ...
  function getCartVersionFromDb($field, $tableName, $where)
  {
    ...
    while (!$_link) {
      ...
      $_link = @mysql_connect($this->Host.":".$this->Port, $this->Username, $this->Password);
      if  (!$_link) {
        sleep(5);
      }
    }

    mysql_select_db($this->Dbname, $_link);
...
class M1_Bridge
{
  ...
  function run()
  {
    ...
    define("M1_BRIDGE_ACTION_CLASS_PATH", M1_STORE_BASE_DIR
                                                . M1_BRIDGE_DIRECTORY_NAME . DIRECTORY_SEPARATOR
                                                . "app" . DIRECTORY_SEPARATOR
                                                . "actions" . DIRECTORY_SEPARATOR
                                                . $action . "_action.class.php" );

    if (!class_exists("M1_Bridge_Action_" . ucfirst($action))) {
      @include_once realpath(M1_BRIDGE_ACTION_CLASS_PATH);
    }
  ...
  function _selfTest()
  {
    ...
    /*if (!is_writable(M1_STORE_BASE_DIR . "/cart2cart/")) {
      die("ERROR_TRIED_TO_PERMISSION_CART2CART" . M1_STORE_BASE_DIR . "/cart2cart/");
    }

    if (!is_writable(M1_STORE_BASE_DIR . "/cart2cart/bridge.php")) {
      die("ERROR_TRIED_TO_PERMISSION_BRIDGE_FILE" . M1_STORE_BASE_DIR . "/cart2cart/bridge.php");
    }*/
...
if ( !class_exists("M1_Config_Adapter") ) {
  @include_once realpath(M1_STORE_BASE_DIR
                . M1_BRIDGE_DIRECTORY_NAME . DIRECTORY_SEPARATOR
                . "app"  . DIRECTORY_SEPARATOR
                . "class" . DIRECTORY_SEPARATOR
