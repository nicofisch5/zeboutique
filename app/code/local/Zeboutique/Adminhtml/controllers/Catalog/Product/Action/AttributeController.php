...
class Zeboutique_Adminhtml_Catalog_Product_Action_AttributeController extends Mage_Adminhtml_Catalog_Product_Action_AttributeController
{
    ...
    public function saveAction()
    {
        ...
        $priceData          = $this->getRequest()->getParam('price', array());
        ...
        try {
            ...
            if ($priceData) {
                foreach ($this->_getHelper()->getProductIds() as $productId) {
                    $currentPrd = Mage::getModel('catalog/product')->load($productId);
                    if ($currentPrd->getCost() > 0) {
                        $currentPrd
                            ->setPrice($priceData['ratio'] * $currentPrd->getCost())
                            ->save();
                    }
                }
            }
