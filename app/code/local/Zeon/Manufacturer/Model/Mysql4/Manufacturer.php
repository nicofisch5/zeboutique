...
class Zeon_Manufacturer_Model_Mysql4_Manufacturer extends Mage_Core_Model_Mysql4_Abstract
{
    ...
    protected function _beforeSave(Mage_Core_Model_Abstract $object)
    {
        if (!$this->getIsUniqueManufacturerToStores($object)) {
            Mage::throwException(Mage::helper('zeon_manufacturer')->__('A manufacturer URL key for specified store already exists.'));
        }

        if ($this->isNumericManufacturerIdentifier($object)) {
            Mage::throwException(Mage::helper('zeon_manufacturer')->__('The Manufacturer URL key cannot consist only of numbers.'));
        }
